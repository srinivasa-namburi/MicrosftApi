...
public static class GreenlightTools
{
    ...
    public static async Task<StartDocumentGenerationResponse> StartDocumentGenerationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var author = TryGetUserOid(httpContextAccessor);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var uploader = request.uploaderOid ?? TryGetUserOid(httpContextAccessor);
    ...
    /// <summary>
    /// Attempts to extract the user's OID from the current HttpContext claims.
    /// Returns null if unauthenticated or no suitable claim is found.
    /// </summary>
