...
/// <summary>
/// Flow MCP Server with only Flow-specific tools for conversational AI interactions.
/// </summary>
public static class FlowMcpServer
{
    /// <summary>
    /// Creates and configures the Flow MCP server application.
    /// </summary>
    public static WebApplication CreateServer(string[] args, int port = 6007)
    {
        var builder = WebApplication.CreateBuilder(new WebApplicationOptions
        {
            Args = args,
            ApplicationName = typeof(FlowMcpServer).Assembly.FullName,
        ...
        // Register plugins and document processes (needed for Flow tools)
        builder.RegisterStaticPlugins(serviceConfigurationOptions);
        builder.RegisterConfiguredDocumentProcesses(serviceConfigurationOptions);

        // Configure MCP server with ONLY Flow tools
        builder.Services
            .AddMcpServer()
            .WithHttpTransport()
        ...
        // Map MCP endpoints at /mcp path to match the routing
        ...
        app.Logger.LogInformation("Flow MCP Server configured on port {Port}", port);
    ...
    private static void ConfigureMiddleware(WebApplication app, IConfiguration configuration)
    {
        ...
        if (disableAuth)
        {
            app.Logger.LogWarning("Flow MCP auth is DISABLED. Do NOT use this in production.");
        }
