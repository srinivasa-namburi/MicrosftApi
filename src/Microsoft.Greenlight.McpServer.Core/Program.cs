...
using Microsoft.Greenlight.McpServer.Core.Auth;
using Microsoft.Greenlight.McpServer.Core.Middleware;
using Microsoft.Greenlight.McpServer.Core.Services;
using Microsoft.Greenlight.McpServer.Core.Tools;
...
// Core MCP Server - Business document processing tools
var builder = WebApplication.CreateBuilder(args);

// Configure Kestrel
// Don't set this explicitly in Kestrel - it relies on launchSettings.json - only used for display purposes.
var port = builder.Configuration.GetValue<int?>("Mcp:HttpPort") ?? 6005;
...
// Configure MCP server with business tools only
builder.Services
    .AddMcpServer()
    .WithHttpTransport()
    .WithTools([typeof(GreenlightTools)]);
...
McpRequestContext.ConfiguredServerNamespace = "business";
...
app.Logger.LogInformation("[CoreMcp] Core MCP Server configured on port {Port}", port);
...
static void ConfigureMiddleware(WebApplication app, IConfiguration configuration)
{
    ...
    {
        var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("CoreMcp");
        using (logger.BeginScope("[CoreMcp]"))
        {
