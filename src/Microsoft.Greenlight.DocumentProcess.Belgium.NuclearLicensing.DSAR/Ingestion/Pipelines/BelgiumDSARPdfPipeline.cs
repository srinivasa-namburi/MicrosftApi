...
using BoundingRegion = Microsoft.Greenlight.Shared.Models.BoundingRegion;

namespace Microsoft.Greenlight.DocumentProcess.Belgium.NuclearLicensing.DSAR.Ingestion.Pipelines;

public class BelgiumDSARPdfPipeline : IPdfPipeline
{
    ...
    private readonly ILogger<BelgiumDSARPdfPipeline> _logger;

    private const string LineSeparator = "------------------------------------------------------------------";

    public BelgiumDSARPdfPipeline(
        IContentTreeProcessor contentTreeProcessor,
        AzureFileHelper AzureFileHelper,
        DocumentAnalysisClient documentAnalysisClient,
        IMapper autoMapper,
        ILogger<BelgiumDSARPdfPipeline> logger)
    {
        ...
        DocumentProcessOptions documentProcessOptions)
    {
        ...
        // If the classification is no-classification, we force processing, as this 
        // document process does not do classification.
        if (document.ClassificationShortCode != "no-classification")
        {
            _logger.LogWarning("BelgiumDSARPdfPipeline: Processing stopped because of unsupported classification");
    ...
    private List<ContentNode> ProcessPdfWithHierarchicalNumberedChapters(ICollection<DocumentParagraph> paragraphList)
    {
        ...
        foreach (var paragraph in paragraphList)
        {
            ...
            var boundingRegions = new List<BoundingRegion>();
