...
public class USNRCEnvironmentalReportAiCompletionService : IAiCompletionService
{
    private const string ProcessName = "US.NRC.EnvironmentalReport";

    private readonly ServiceConfigurationOptions _serviceConfigurationOptions;
    private readonly OpenAIClient _openAIClient;
    private readonly DocGenerationDbContext _dbContext;
    private readonly ILogger<USNRCEnvironmentalReportAiCompletionService> _logger;
    private readonly IServiceProvider _sp;
    private Kernel? _sk;
    private readonly int _numberOfPasses = 6;

    public USNRCEnvironmentalReportAiCompletionService(
        IOptions<ServiceConfigurationOptions> serviceConfigurationOptions,
        [FromKeyedServices("openai-planner")] OpenAIClient openAIClient,
        DocGenerationDbContext dbContext,
        IServiceProvider sp, 
        ILogger<USNRCEnvironmentalReportAiCompletionService> logger)
    {
    ...
    private async Task<string> SummarizeOutput(string originalContent)
    {
        ...
        var systemPrompt = """
                           ...
                           environmental reports for Small Modular nuclear Reactors ('SMR') and one or more participants.
                           The AI has been trained on GPT-4 LLM data through to April 2023 and has access to additional data on more recent
                           output samples from previous environmental reports and supporting documents submitted to the NRC. 
                           Provide responses that can be copied directly into an environmental report. 
                           """;
