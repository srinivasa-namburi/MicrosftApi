...
public class ConfigurationApiClient : WebAssemblyBaseServiceClient<ConfigurationApiClient>, IConfigurationApiClient
{
    ...
    {
        var response = await SendGetRequestMessage($"/api/systemprompt/{Uri.EscapeDataString(promptName)}", authorize: true);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await SendGetRequestMessage($"/api/systemprompt/{Uri.EscapeDataString(promptName)}/default", authorize: true);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<SystemPromptResponse>() ??
    ...
    public async Task UpdateSystemPromptAsync(string promptName, string text, bool isActive = true)
    {
        ...
        var response = await SendPutRequestMessage($"/api/systemprompt/{Uri.EscapeDataString(promptName)}", request, authorize: true);
        response.EnsureSuccessStatusCode();
    }

    public async Task DeleteSystemPromptOverrideAsync(string promptName)
    {
        var response = await SendDeleteRequestMessage($"/api/systemprompt/{Uri.EscapeDataString(promptName)}", authorize: true);
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task<PromptValidationResult> ValidateSystemPromptAsync(string promptName, string text)
    {
        ...
        var response = await SendPostRequestMessage($"/api/systemprompt/{Uri.EscapeDataString(promptName)}/validate", request, authorize: true);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await SendGetRequestMessage("/api/systemprompt/available", authorize: true);
        response.EnsureSuccessStatusCode();
