...
{
    public class DocumentLibraryApiClient : WebAssemblyBaseServiceClient<DocumentLibraryApiClient>, IDocumentLibraryApiClient
    {
        ...
        {
            var response = await SendPostRequestMessage($"/api/document-libraries/{documentLibraryId}/document-processes/{documentProcessId}/associate", null);
            response.EnsureSuccessStatusCode();
        }

        public async Task DisassociateDocumentProcessAsync(Guid documentLibraryId, Guid documentProcessId)
        {
            var response = await SendPostRequestMessage($"/api/document-libraries/{documentLibraryId}/document-processes/{documentProcessId}/disassociate", null);
            response.EnsureSuccessStatusCode();
        }
    }
}
