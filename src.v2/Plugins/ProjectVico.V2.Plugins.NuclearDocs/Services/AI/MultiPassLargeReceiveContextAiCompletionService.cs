...
namespace ProjectVico.V2.Plugins.NuclearDocs.Services.AI;
...
public class MultiPassLargeReceiveContextAiCompletionService : IAiCompletionService
{
    ...
    public async Task<List<ContentNode>> GetBodyContentNodes(List<ReportDocument> documents, string sectionOrTitleNumber, string sectionOrTitleText, ContentNodeType contentNodeType, string tableOfContentsString)
    {
        ...
        for (int i = 0; i < _numberOfPasses; i++)
        {
            ...
            {
                chatResponses[chatResponses.Count - 1] = chatResponses.Last().Replace("[*TO BE CONTINUED*]", "", StringComparison.InvariantCultureIgnoreCase);
            }
        ...
        StringBuilder combinedBodyTextStringBuilder = new StringBuilder();
