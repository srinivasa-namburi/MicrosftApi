...
using ProjectVico.V2.Plugins.Earthquake.Models;
using FeatureCollection = BAMCIS.GeoJSON.FeatureCollection;

namespace ProjectVico.V2.Plugins.Earthquake.Connectors;
...
public class USGSEarthquakeConnector : IEarthquakeConnector
{
        ...
        double minMagnitude, DateOnly minDate, DateOnly maxDate, string sortOrder = "time")
    {
        List<EarthquakeDetail> searchResult =
            await CallUSGSEarthquakeAPI(latitude, longitude, radius, maxResults, minMagnitude, minDate, maxDate, sortOrder);
        ...
        using (var client = new HttpClient())
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                foreach (var feature in featureCollection.Features)
                {
                    ...
                    {
                        Console.WriteLine("Received a bad record from the USGS API - Ignoring. For reference, here is the record:");
