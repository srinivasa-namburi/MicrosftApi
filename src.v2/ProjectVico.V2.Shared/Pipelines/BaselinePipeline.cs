...
namespace ProjectVico.V2.Shared.Pipelines;
...
public class BaselinePipeline : IPdfPipeline
{
    ...
    public async Task<IngestionPipelineResponse> RunAsync(string blobUrl, string pdfName)
    {

        var response = new IngestionPipelineResponse();
        var stream = await _azureFileHelper.GetFileAsStreamFromFullBlobUrlAsync(blobUrl);
        stream!.Position = 0;
        ...
        var documentIntelligenceAnalysisResult = await AnalyzePdfWithDocumentIntelligenceAsync(pdfName, stream);
        ...
        AnalyzeResult documentIntelligenceAnalysisResult)
    {
        ...
        foreach (var duplicateParagraph in duplicateParagraphs)
        {
            ...
            paragraphsToRemove.AddRange(duplicateParagraphsInDocument);
